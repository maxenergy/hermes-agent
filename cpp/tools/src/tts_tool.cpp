...
std::optional<std::string> decode_base64(const std::string& b64) {
    // C++17-friendly lookup table builder: a function-local static std::array
    // is initialized once on first call.
...
std::string provider_mistral(const std::string& text,
    ...
    std::string model =
        opts.value("model", std::string(kTtsDefaultMistralTtsModel));
...
std::string handle_tts(const nlohmann::json& args, const ToolContext& ctx) {
    ...
    text = cap_text_length(text, kTtsMaxTextLength);
