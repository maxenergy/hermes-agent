...
void McpClientManager::register_server_tools(const std::string& server_name,
    ...
    // Register a single stub tool for this MCP server.
    // Phase 12 will replace this with actual tool discovery via the
    // MCP protocol's tools/list method.
    ToolEntry e;
    ...
    e.handler = [server_name](const json& /*args*/,
        ...
        return tool_error("MCP server not connected: " + server_name +
                          " (transport will be implemented in Phase 12)");
    };
